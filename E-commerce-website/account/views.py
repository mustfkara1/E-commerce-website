...
def login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.add_message(request, messages.ERROR,'Hatalı Kullanıcı Adı Yada Parola Girdiniz')
        ...
        if password == repassword:
            ...
            if User.objects.filter(username = username).exists():
                ...
                if User.objects.filter(email = email).exists():
                    ...
                    user.save()
                    messages.add_message(request, messages.SUCCESS,'Kullanıcı Başarı İle Oluşturuldu')
                    return redirect('login')

        else:
            print('Parolalarınız Eşleşmiyor Değil')
            return redirect('register')
    else:
